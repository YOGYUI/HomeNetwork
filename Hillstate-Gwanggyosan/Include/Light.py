...
if __name__ == '__main__':
    def printPacket(p: bytearray):
        print(' '.join(['%02X' % x for x in p]))

    dev = Light(name='test', index=0, room_index=6)
    packet1 = dev.queryState()
    printPacket(packet1)
    packet2 = dev.setState(True)
    printPacket(packet2)
    packet3 = dev.setState(False)
    printPacket(packet3)